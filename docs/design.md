...
- **select** — `choose from list` 대체. 제목 + 설명 + 스크롤 가능한 리스트 + 취소/기본 버튼.
  ...
  - 항목을 구간 헤더로 묶어 보여줄 수 있다(`shared/types.ts`의 `SelectItem.group`, 예: gtd-reminders의 업무/개인). 호출부는 `hackmac-popup`에 `"그룹"$'\x1f'"라벨"` 형태로 항목을 넘기고(구분자 없이 넘기면 기존처럼 민짜 리스트), 같은 그룹이 연속되면 헤더 하나만 그려진다 — 선택/기본값 비교와 스크립트로 돌아가는 값은 항상 라벨 쪽이라 그룹 표시가 호출부 로직에 영향을 주지 않는다.
