...
이 Mac으로 할 수 있는 일을 늘려주는 서비스 스토어. 카드를 눌러 서비스를 켜고 끌 뿐, 단축키를 직접 만들지 않는다.
...
5. 단축키를 눌러 서비스를 쓰는 도중 선택/입력이 필요한 순간(예: GTD 리스트에서 하나 고르기)도 macOS 기본 다이얼로그가 아니라 앱과 같은 톤으로 그린 팝업이 뜬다 — 스토어 화면과 팝업이 하나의 디자인 시스템([docs/design.md](docs/design.md))을 공유한다.
...
스크립트가 사용자에게 고르거나 입력받아야 할 때는 osascript 다이얼로그 대신 `resources/bin/hackmac-popup`을 불러 HackMac 앱에게 팝업을 그려달라고 요청한다. 자세한 구조와 이유는 [CLAUDE.md의 "서비스 설계 원칙"](CLAUDE.md#서비스-설계-원칙), 디자인 토큰은 [docs/design.md](docs/design.md) 참고.

## 지금 들어있는 서비스

- **미리알림 GTD** (`gtd-reminders`) — David Allen의 GTD를 macOS 미리알림 위에서 구현한 todo 관리. 업무 등록(⌥1)·시작(⌥2)·완료(⌥3)·삭제(⌥4)·현황보기(⌥9)·공유하기(⌥0)·프로젝트 등록(⌥\)·업무 구조 정규화(⌃⌥0) 여덟 단축키. 프로젝트는 업무/개인으로 나뉘어 팝업·현황·공유 전반에서 따로 표시되고, 개인 프로젝트는 공유 대상에서 빠진다.
